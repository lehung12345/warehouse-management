...
func SetupRoutes(r *gin.Engine, db *gorm.DB) {
	// PUBLIC ROUTES (không cần auth)
	controllers.RegisterAuthRoutes(r, db) // /auth/login

	// API ROUTES (có auth) - dùng chung cho web admin và Flutter
	api := r.Group("/api")
	api.Use(middleware.AuthMiddleware())
	{
		...
		{
			staff.GET("/me", func(c *gin.Context) {
				c.JSON(200, gin.H{
					"user_id":  c.GetUint("userID"),
					"username": c.GetString("username"),
					"role":     c.GetString("role"),
		...
		// Các route nghiệp vụ
		InventoryRoutes(api, db)
		LocationRoutes(api, db)
		...
		{
			controllers.RegisterUserManagementRoutes(admin, db)
		}
	}
}